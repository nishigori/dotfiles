...
syn keyword pentadactylCommand loadplugins lpl gr[oup] ! run Clistk[eys] Clk
    ...
    \ bw[ipeout] bun[load] tabc[lose] blistk[eys] blk bm[ap] bma[rk] bmarks
    \ bno[remap] b[uffer] buffers files ls tabs bunm[ap] ca[bbreviate] cd
    \ chd[ir] clistk[eys] clk cm[ap] cno[remap] colo[rscheme] com[mand]
    \ comp[letions] contexts cookies ck cuna[bbreviate] cunm[ap] delbm[arks]
    \ delc[ommand] delg[roup] delmac[ros] delm[arks] delqm[arks] dels[tyle]
    \ dia[log] dlc[lear] doautoa[ll] do[autocmd] downl[oads] dl ec[ho] echoe[rr]
...
syn keyword pentadactylOption activate act altwildmode awim autocomplete au
    ...
    \ showtabline stal spelllang spl strictfocus sf suggestengines timeoutlen
    \ tmol titlestring urlseparator urlsep us verbose vbs wildanchor wia
